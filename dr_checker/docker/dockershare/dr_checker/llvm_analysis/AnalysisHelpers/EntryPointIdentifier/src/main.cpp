...
//                if(kernel_sts[i].st_name == containedType->getStructName()) {
                    ...
                    if(currGlobal->hasInitializer()) {
                        ...
                        cout << string(containedType->getStructName()) << endl;
                        if (actualStType != nullptr) {
                            unsigned operandnum = actualStType->getNumOperands();
                            for (unsigned index = 0; index < operandnum; index ++) {
                                Value *currFieldVal = actualStType->getOperand(index);
                                Function *targetFunction = dyn_cast<Function>(currFieldVal);
                                if (targetFunction == nullptr)
                                    cout << "nullptr:" << index << endl;
                                else if (targetFunction->isDeclaration())
                                    cout << "isDeclaration:" << index << endl;

                                if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                                    cout << "fucking found -----------------------------------" << index << endl;
                                    cout << targetFunction->getName().str().c_str();
                                    fprintf(outputFile, "%s:%s\n", "NEWTYPE", targetFunction->getName().str().c_str());
                                }
                            }
                        } else {
                            cout << "null ptr" << endl;

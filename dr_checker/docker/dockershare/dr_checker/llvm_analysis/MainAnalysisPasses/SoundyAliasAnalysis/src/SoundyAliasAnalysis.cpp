...
#define NEW_TYPE "NEWTYPE"
    ...
    private:
        ...
        void setupFunctionArgs(Function *targetFunction, GlobalState &targetState, std::vector<Instruction *> *callSites) {
            ...
            if (functionType == NEW_TYPE)
            {
                /*unsigned long tmp = 0;
                for(auto i = targetFunction->getArgumentList().begin(); i != targetFunction->getArgumentList().end(); ++i)
                {
                    
                    if(i->getType()->isPointerTy())
                    {
                        cout << tmp << endl;
                        pointerArgs.insert(tmp);
                    } 
                    tmp ++;
                }*/
                cout << "????????????????????" << endl;

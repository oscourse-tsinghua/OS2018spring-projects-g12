...
class ParseHeaders(Component):
    ...
    def perform(self):
        ...
        os.system("grep -rl \'operations {\' " + str(self.kernel_src_dir) + " > " + str(temp_hdr_file_list))
        os.system("grep -rl \'ops {\' " + str(self.kernel_src_dir) + " >> " + str(temp_hdr_file_list))
        output_file_size = 0
...
def _run_single_c2xml((c2xml_bin, curr_hdr_file, dst_work_dir, all_hdr_options)):
    ...
    cmd_line = c2xml_bin + " " + cmd_line + " " + curr_hdr_file + " > " + target_out_file + " 2>&1"
    print cmd_line
    back_wd = os.getcwd()

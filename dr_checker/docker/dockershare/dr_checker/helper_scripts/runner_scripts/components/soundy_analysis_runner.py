...
def _run_soundy_checker(combined_arg):
    ...
    tmp = timeout_prefix + opt_bin_path + " -analyze -debug -load \"" + so_path + '\" ' + '-dr_checker -toCheckFunction=\"' + str(func_name) + '\" ' + '-functionType=\"' + ep_type + '\" '+ '-skipInit=1 -outputFile=\"' + output_json_file + '\" ' + '-instrWarnOutput=\"' + instr_json_file + '\" ' + bc_file_name + ' >> ' + output_total_file + ' 2>&1'
    log_info(tmp)
    ret_val = os.system(timeout_prefix + opt_bin_path + " -analyze -debug -load \"" + so_path + '\" '
   ...
   # log_info(ret_val)
    '''
    [*]  timeout 45m /dr_checker/drcheckerdeps/llvm/build/bin/opt -analyze -debug -load "/dr_checker/dr_checker/helper_scripts/../llvm_analysis/MainAnalysisPasses/build_dir/SoundyAliasAnalysis/libSoundyAliasAnalysis.so" -dr_checker     -toCheckFunction="dev_open" -functionType="NEWTYPE" -skipInit=1 -outputFile="/dockershare/dr_checker_out/dev_open.json" -instrWarnOutput="/dockershare/dr_checker_out/instr_warnings/dev_openinstr_warngs.json" /tmp/tmpFRUd6g >> /dockershare/dr_checker_out/dev_open.output 2>&1
    '''
